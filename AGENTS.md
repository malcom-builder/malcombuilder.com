...
- **Resplandor del Hero suavizado, posicionado y optimizado:** Se definió una clase CSS `.hero-orb` para evitar que el compilador descarte los estilos como código muerto y se posicionó en la esquina superior derecha (`top: -80px`, `right: -100px` en desktop; `top: -50px`, `right: -80px` en mobile), bajándola ligeramente para que la parte central del resplandor de fondo (el núcleo del gradiente) sea visible debajo de la barra de navegación. Se corrigieron los parpadeos del loop ajustando los keyframes de opacidad (`0.07 -> 0.15 -> 0.07`) y escala (`0.96 -> 1.04 -> 0.96`).
