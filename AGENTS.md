...
- **Soporte Total a Temas (Light / Dark Mode en Brief):** Reemplazados todos los colores blancos estáticos y sombreados hardcoded por variables de tema adaptativas (`text-[var(--color-fg)]`, `bg-[var(--color-accent)]/10`, `rgba(var(--spotlight-color), ...)`). El fondo de la página se adaptó a `bg-[var(--color-bg)]`, resolviendo la invisibilidad de elementos en Modo Light.
...
- **Restauración de resplandores/glows entre secciones:** Se eliminó `overflow: "hidden"` de todas las secciones (`#about` y `#cta` incluidos) para permitir que los orbes de fondo fluyan y se fusionen verticalmente sin cortes bruscos. El scroll horizontal en dispositivos móviles se contuvo de forma global mediante `overflow-x: clip` en los selectores `body` y `main` in `globals.css`, lo que evita la barra lateral sin interferir con el desbordamiento vertical de los efectos de iluminación.
- **Resplandor del Hero suavizado, posicionado y optimizado:** Se definió una clase CSS `.hero-orb` para evitar que el compilador descarte los estilos como código muerto y se posicionó y bajó en la pantalla (`top: 15%`, `right: 5%` de manera global), de forma que la parte central del resplandor de fondo (el núcleo del gradiente) sea plenamente visible y no quede oculta detrás de la barra de navegación, recortada en la parte superior o desplazada muy al borde. Se corrigieron los parpadeos del loop ajustando los keyframes de opacidad (`0.07 -> 0.15 -> 0.07`) y escala (`0.96 -> 1.04 -> 0.96`).
